...
from tabnets.tabnet_pytorch import TabNet
...
if __name__ == '__main__':
    ...
    # y_train = y_train.reshape(-1, 1)  # 回归
    # y_test = y_test.reshape(-1, 1)     # 回归
    ...
    model.fit(
        ...
        eval_metric=['accuracy', 'balanced_accuracy', 'logloss'], # 'auc'    # 回归 ['rmsle', 'mae', 'rmse', 'mse'],  multiclass classification : 'accuracy', 'balanced_accuracy', 'logloss'
        max_epochs=10,
        patience=50,
    ...
    # 先保存到pth，然后加载pth模型,再转onnx

    inputs_shape = Variable(torch.randn(1, 4))
    onnx_out = torch.onnx.export(model.network, inputs_shape, 'iris_reg.onnx', export_params=True, verbose=True,
                                 input_names=['input'], output_names=['output'],
                                 dynamic_axes={"input": {0: "batch_size"}, "output": {0: "batch_size"}})

    session = ort.InferenceSession('iris_reg.onnx')
    in_name = session.get_inputs()[0].name
    out_name = session.get_outputs()[0].name

    x = X_test[:1].astype('float32')

    onnx_out = session.run([out_name], {in_name: x})
    model_pred = model.predict(torch.tensor(x))
    print(onnx_out)
    print(model_pred)
